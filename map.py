...
if selected_page == "대시보드":
    ...
    if filtered_df.empty:
        ...
        trend_text = "연도별 비교 자료 부족"
        if len(dashboard_year_counts) >= 2:
            first_count = int(dashboard_year_counts.iloc[0]["사고건수"])
            last_count = int(dashboard_year_counts.iloc[-1]["사고건수"])
            first_year = int(dashboard_year_counts.iloc[0]["연도"])
            last_year = int(dashboard_year_counts.iloc[-1]["연도"])
            if first_count > 0:
                trend_rate = (last_count - first_count) / first_count * 100
                trend_direction = "증가" if trend_rate > 0 else "감소" if trend_rate < 0 else "동일"
                trend_text = (
                    f"{first_year}년 대비 {last_year}년 "
                    f"{abs(trend_rate):.1f}% {trend_direction}"
                )
