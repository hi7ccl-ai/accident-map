...
10. File Search에서 참고한 파일의 내부 인용표시(filecite)는 최종 보고서에 출력하지 않는다.
""".strip()
...
def _remove_file_search_citations(text):
    """File Search가 응답에 삽입한 내부 filecite 표기를 화면에서 제거한다."""
    cleaned_text = re.sub(
        r"filecite.*?",
        "",
        str(text or ""),
    )
    return re.sub(r"[ \t]{2,}", " ", cleaned_text)


def _remove_markdown_heading_numbers(text):
    """자동 번호 디자인과 중복되는 Markdown 2단계 제목 번호를 제거한다."""
    return re.sub(
        r"(?m)^(##\s+)\d+\.\s*",
        r"\1",
        str(text or ""),
...
# ============================================================
    ...
    display_text = _remove_file_search_citations(display_text)
    display_text = _remove_markdown_heading_numbers(display_text)
...
# ============================================================
# AI 보고서 영구 저장(Supabase)
# - 보고서 종류별 최신 결과 1개를 저장
# - 새 세션·새로고침·앱 재시작 시 저장 결과를 다시 복원
# ============================================================

def _get_supabase_report_config():
    """Streamlit Secrets에서 Supabase 접속정보를 읽는다."""
    try:
...
def _request_supabase_reports(method="GET", payload=None, query=None):
    """Supabase Data API의 ai_reports 테이블을 호출한다."""
    supabase_url, supabase_secret_key = _get_supabase_report_config()

    if not supabase_url or not supabase_secret_key:
        raise RuntimeError(
            "Streamlit Secrets에 SUPABASE_URL과 "
            "SUPABASE_SECRET_KEY를 등록하세요."
        )

    endpoint = f"{supabase_url}/rest/v1/ai_reports"
    if query:
    ...
    try:
        ...
        raise RuntimeError(
            f"Supabase 요청 실패(HTTP {error.code}): "
            f"{error_detail[:500]}"
        ) from error
    except urllib.error.URLError as error:
        raise RuntimeError(
            f"Supabase 연결 실패: {error.reason}"
        ) from error
...
def load_persistent_ai_reports():
    """Supabase에 저장된 보고서 4종을 현재 Session State로 복원한다."""
    rows = _request_supabase_reports(
        method="GET",
        query={
            "select": (
                "report_type,report_text,filter_signature,"
                "filter_info,usage_info,updated_at"
            ),
    ...
    for row in rows:
        ...
        if report_type not in allowed_report_types or not report_text:
            continue

        st.session_state[f"ai_result_{report_type}"] = report_text
        st.session_state[f"ai_usage_{report_type}"] = (
            row.get("usage_info") or {}
        )
        st.session_state[f"ai_signature_{report_type}"] = (
            row.get("filter_signature")
        )
        st.session_state[f"ai_filters_{report_type}"] = (
            row.get("filter_info") or {}
        )
        restored_types.append(report_type)

    if (
        restored_types
        and st.session_state.get("selected_ai_report_type")
        not in restored_types
    ):
        # 조회결과가 최신순이므로 가장 최근 보고서를 기본 표시
        st.session_state["selected_ai_report_type"] = restored_types[0]

    return len(restored_types)


def save_persistent_ai_report(
    report_type,
    report_text,
    filter_signature,
    filter_info,
    ...
    """새로 생성한 보고서로 해당 종류의 Supabase 저장본만 교체한다."""
    payload = {
        "report_type": report_type,
    ...
    _request_supabase_reports(
        method="POST",
        payload=payload,
        query={"on_conflict": "report_type"},
    )
...
def parse_natural_language_filters(user_query, filter_catalog):
    ...
    try:
        ...
        latest_year_start = f"{latest_period_for_example.year}-01"
    except Exception:
        recent_three_year_start = "최신월에서 35개월 전"
        latest_year_start = "최신 데이터 연도의 1월"
...
6. 기간 표현
   - '금년도', '금년', '올해'는 available_period의 마지막 값이 속한 연도로 해석한다.
   - 시작월은 해당 연도의 1월이고 종료월은 available_period의 마지막 값이다.
     현재 데이터에서 '금년도' = start_year_month="{latest_year_start}",
...
""".strip()
    ...
    parsed_result = _supplement_current_year_from_query(
        parsed_result=parsed_result,
        user_query=user_query,
        filter_catalog=filter_catalog,
    )
...
def _supplement_current_year_from_query(
    parsed_result,
    user_query,
    filter_catalog,
):
    """'금년도', '금년', '올해'를 데이터의 최신 연도 기준으로 보정한다."""
    query = re.sub(r"\s+", " ", str(user_query).strip())

    if not re.search(r"(?:금년도|금년|올해)", query):
        return parsed_result
    ...
    parsed_result["start_year_month"] = f"{latest_period.year}-01"
    parsed_result["end_year_month"] = latest_period.strftime("%Y-%m")
...
st.markdown(
    ...
    /* Cloud 테마와 관계없이 사이드바 multiselect를 흰색으로 통일 */
    [data-testid="stSidebar"]
...
.taap-main-header .taap-title-icon {
    ...
    /* Streamlit Community Cloud 대응
       버튼 key에서 생성되는 고유 클래스를 직접 지정하여
       로컬과 동일한 현재 메뉴 글자 크기(1.3rem)를 적용한다. */
    .st-key-header_nav_dashboard button,
    .st-key-header_nav_gis button,
    .st-key-header_nav_statistics button,
    .st-key-header_nav_ai_report button,
    [class*="st-key-header_nav_"] button {
        font-size: 1.3rem !important;
        font-weight: 850 !important;
    }

    .st-key-header_nav_dashboard button p,
    .st-key-header_nav_gis button p,
    .st-key-header_nav_statistics button p,
    .st-key-header_nav_ai_report button p,
    [class*="st-key-header_nav_"] button p {
        font-size: inherit !important;
        font-weight: inherit !important;
    }
...
if selected_page == "AI 리포트":
    ...
    # ------------------------------------------------------------
    # 새 브라우저 세션에서 Supabase 저장 보고서를 한 번만 복원
    # ------------------------------------------------------------
    if not st.session_state.get("_persistent_ai_reports_loaded", False):
        supabase_url, supabase_secret_key = _get_supabase_report_config()

        if supabase_url and supabase_secret_key:
            try:
                load_persistent_ai_reports()
                st.session_state.pop("_ai_report_load_error", None)
            except Exception as error:
                st.session_state["_ai_report_load_error"] = (
                    f"{type(error).__name__}: {error}"
                )

        st.session_state["_persistent_ai_reports_loaded"] = True

    if st.session_state.get("_ai_report_load_error"):
        st.warning(
            "저장된 AI 보고서를 불러오지 못했습니다. "
            "새로고침 후에도 계속되면 Supabase 설정을 확인하세요."
        )
        st.code(st.session_state["_ai_report_load_error"])

    if st.session_state.get("_ai_report_save_error"):
        st.warning(
            "방금 생성한 보고서는 현재 화면에 보관되지만, "
            "영구 저장에는 실패했습니다."
        )
        st.code(st.session_state["_ai_report_save_error"])
    ...
    def queue_ai_report(report_type):
        ...
        - 현재 브라우저 세션에서 인증한 경우 즉시 생성 대기
        - 인증하지 않은 경우 비밀번호 입력 대상으로 저장
        - 기존 저장 보고서 열람에는 영향을 주지 않음
        """

        if st.session_state.get("ai_report_unlocked", False):
            st.session_state["selected_ai_report_type"] = report_type
            st.session_state["pending_ai_report_type"] = report_type
            return

        st.session_state["ai_report_auth_target"] = report_type
    ...
    # ------------------------------------------------------------
    # AI 보고서 생성 비밀번호 확인
    # - 기존 저장 보고서는 비밀번호 없이 열람 가능
    # - 새 보고서 생성에만 비밀번호 적용
    # - 한 번 인증하면 현재 브라우저 세션 동안 재입력 생략
    # ------------------------------------------------------------
    auth_target = st.session_state.get("ai_report_auth_target")

    if auth_target is not None:
        auth_report_info = ai_report_types.get(auth_target, {})

        with st.form(
            "ai_report_password_form",
            clear_on_submit=True,
        ):
            st.info(
                f"'{auth_report_info.get('card_title', 'AI 보고서')}'를 "
                "새로 작성하려면 비밀번호를 입력하세요."
            )

            entered_report_password = st.text_input(
                "보고서 작성 비밀번호",
                type="password",
                placeholder="비밀번호 입력",
            )

            auth_submit_col, auth_cancel_col = st.columns(2)

            with auth_submit_col:
            ...
            with auth_cancel_col:
                password_cancelled = st.form_submit_button(
                    "취소",
                    use_container_width=True,
                )

        if password_submitted:
            configured_report_password = str(
                st.secrets.get("AI_REPORT_PASSWORD", "")
            )

            if not configured_report_password:
                st.error(
                    "Streamlit Secrets에 AI_REPORT_PASSWORD를 "
                    "등록하세요."
                )
            elif hmac.compare_digest(
                entered_report_password,
                configured_report_password,
            ):
                st.session_state["ai_report_unlocked"] = True
                st.session_state["selected_ai_report_type"] = auth_target
                st.session_state["pending_ai_report_type"] = auth_target
                st.session_state.pop("ai_report_auth_target", None)
                st.rerun()
            else:
    ...
    if clicked_report_type is not None:
        ...
        try:
            ...
            # Supabase 영구 저장
            # - 저장 실패와 AI 생성 성공을 분리
            # - 실패해도 현재 세션의 생성 결과는 유지
            # ----------------------------------------------------
            try:
                save_persistent_ai_report(
                    report_type=clicked_report_type,
                    report_text=ai_result,
                    filter_signature=current_filter_signature,
                    filter_info=selected_filter_info,
                    usage_info=ai_usage,
                )
                st.session_state.pop("_ai_report_save_error", None)
            except Exception as persistence_error:
                st.session_state["_ai_report_save_error"] = (
    ...
    if (
        ...
        with st.expander(" ", expanded=True):
            ...
            with report_content_col:
                ...
                ai_display_text = _remove_file_search_citations(
                    st.session_state[
                        f"ai_result_{selected_ai_report_type}"
                    ]
                )
                ai_display_text = _remove_markdown_heading_numbers(
                    ai_display_text
                )
...
# ============================================================
    ...
    def render_statistics_chart_title(title):
        """통계분석 세부 그래프 제목을 동일한 크기와 색상으로 출력한다."""
        st.markdown(
            f"""
            <div style="
                font-size:1.2rem;
    ...
    if not filtered_df.empty:
        ...
        # --------------------------------------------------------
        with row1_col1:
            render_statistics_chart_title("📅 발생 연도별 추이")
        ...
        # --------------------------------------------------------
        with row1_col2:
            render_statistics_chart_title("📂 부상 정도")
        ...
        # --------------------------------------------------------
        with row1_col3:
            render_statistics_chart_title("🚑 사고 종별")
        ...
        # 가해차량 차종
            # --------------------------------------------------------
            with row2_col1:
                render_statistics_chart_title("🚗 가해차량 차종")
        ...
        # 피해차량 차종
            # --------------------------------------------------------
            with row2_col2:
                render_statistics_chart_title("🚙 피해차량 차종")
        ...
        # --------------------------------------------------------
            with row3_col1:
                render_statistics_chart_title("🛑 사망자 유형")
            ...
            # --------------------------------------------------------
            with row3_col2:
                render_statistics_chart_title("👤 사망자 연령대")
        ...
        with st.expander(" ", expanded=True):
            ...
            # --------------------------------------------------------
            with row4_col1:
                render_statistics_chart_title("📅 요일별")
            ...
            # --------------------------------------------------------
            with row4_col2:
                render_statistics_chart_title("⏰ 시간대별")
            ...
            # --------------------------------------------------------
            with row4_col3:
                render_statistics_chart_title("⚖️ 법규위반별")
